    ...
    for(const auto& wall_rotation: wall_rotations){
        for(const auto& rot_pair: T_O_Ogr_rotations_dict){
            ...
            std::pair<Eigen::Isometry3d, Eigen::Isometry3d> green_cube_3_grasp_pair =  {
                    T_B_O_3 * wall_rotation * rotation * T_arm7_link_pre_gr_O.inverse() * T_arm7_link_arm_tool_link,
                    T_B_O_3 * wall_rotation * rotation  * T_arm7_link_gr_O.inverse() * T_arm7_link_arm_tool_link};
